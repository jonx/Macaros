...
## Desktop icons (`.info`) and themes — the skipped payload

Booting the build dir directly also skips the **icon set** and **theme/wallpaper**
payload. The prefs/tools/utilities **executables** build into `AROS/Prefs`,
`AROS/Tools`, etc., but their `.info` icons do **not** — icons are a separate tree
(`images/IconSets/`, default set **Gorilla**) that only rides in via the
`workbench`/`workbench-complete` metatargets, never via the narrow `-quick`
targets this port builds. Symptom: **Wanderer draws an empty Prefs/Tools/Utilities
drawer** (executables present but iconless), and `Wanderer → AROS → GUI Settings…`
(→ `OpenWorkbenchObject("SYS:Prefs/Zune")`) launches nothing. `.info` files are
generated by the host tool `ILBMTOICON` from `<Name>.info.src` + `<Name>.png` and
associate to executables **by name**.

Generate them straight into the boot tree (each writes into `AROSDIR`, i.e. the
...
This takes the tree from ~11 `.info` to ~230 (all drawer + program icons) and
populates every theme (AROSDefault, Ice, Darkness, AmigaOS1/3/4.x, …). The
graphics editors themselves are `SYS:Prefs/ScreenMode` (resolution/depth) and
`SYS:Prefs/Monitor`; GUI look-and-feel is `SYS:Prefs/Zune`.
