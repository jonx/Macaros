...
| clone (per-open) library bases | NOT done, and reported by name: a guest library whose `Open` returns a different base per opener is refused rather than mis-bookkept. Photogenics's `pgsdemo.library` is the first to need it |
