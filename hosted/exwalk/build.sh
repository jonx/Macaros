#!/bin/bash
# build.sh -- cross-build C:ExWalk (the concurrent ExNext/ExAll emul-handler
...
#
# Output: hosted/exwalk/ExWalk (copy into the boot image's C: to deploy).
set -eu
...
"$CLANG" --target=aarch64-unknown-aros -mcmodel=large -ffixed-x18 -O2 -Wall -Wno-pointer-sign \
    ...
    "$LIBDIR/startup.o" "$DIR/exwalk.c" -o "$OUT" \
