...
int main(void)
{
    ...
    printf("[apps68k] runner: REAL vasm-assembled 68k hunk programs, ALL FOUR through the "
           "[J5d] JIT (Emu68's REAL decoders + our dispatcher + the (An) EA edit + the "
           "[J3] library bridge), each byte-exact vs an independent reference.\n\n");
    ...
    printf("[apps68k] PASS: all four REAL 68k programs (mul=42, fact=120, arraysum=150, "
           "libcall=0) ran THROUGH THE JIT via Emu68's REAL per-opcode decoders + our "
           "re-hosted dispatcher; each register file + sandbox memory is byte-exact vs an "
