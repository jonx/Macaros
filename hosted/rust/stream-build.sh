#!/bin/bash
# stdnet-build.sh -- link the std probe + the bsdsocket net glue + rs_stream_main.c
# into C:RustStream (the RSN std::net::TcpStream round-trip). Same recipe as
# net-build.sh; the only change is the harness (rs_stream_main.c) and the output
...
echo "[stream] compile glue + harness (-ffixed-x18)"
...
"$CC" "${CFLAGS[@]}" -c "$DIR/rs_stream_main.c" -o "$OUT/rs_stream_main.o"

echo "[stream] link RustStream"
COMPILER_PATH="$XTBIN" "$COLLECT" \
    --eh-frame-hdr --allow-multiple-definition \
    -L"$LIBDIR" -L"$XTLIB" -o "$OUT/RustStream" \
    "$LIBDIR/startup.o" "$OUT/rs_stream_main.o" "$OUT/aros_net_glue.o" "$OUT/aros_fs_glue.o" "$OUT/aros_process_glue.o" "$OUT/aros_thread_glue.o" "$OUT/aros_sync_glue.o" "$OUT/aros_env_glue.o" "$RSLIB" \
    -\( "${AUTOLIB[@]}" "${STDLIBS[@]}" -\)
echo "[stream] built: $OUT/RustStream ($(stat -f%z "$OUT/RustStream" 2>/dev/null) bytes)"
cp -f "$OUT/RustStream" "$CDIR/RustStream"; chmod +x "$CDIR/RustStream"
echo "[stream] deployed -> $CDIR/RustStream"
