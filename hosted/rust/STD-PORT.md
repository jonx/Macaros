...
| `process/aros.rs` | done (**live pipes** + output/status + **env/cwd**) | `Command` -> a shell command line run by dos `SystemTagList`. **Streaming child stdio** (2026-07-25): `Stdio::piped()` wires the child to `PIPE:` endpoints and `SystemTagList(SYS_Asynch)` returns while it runs, so you can read a child's stdout mid-run and write its stdin; `wait()` blocks on the child's exit signal (`NP_ExitCode` hook) rather than polling. `output()` uses the same path (no more temp files). Two traps: AROS's Shell ends `return error ? RETURN_FAIL : RETURN_OK`, so the real exit code must be read from `cli_ReturnCode` in the exit hook; and an unset stdin must be `NIL:` when the caller does not need it, or a child that reads stdin blocks forever on the console. Still missing: `kill()` (no safe way to tear down an AROS Process), and `try_clone` on an endpoint (the handler allows one reader + one writer). |
...
| `fs` `link`/`copy`, `net` timeouts, … | **stubs / documented gaps** | a few corners fall through to `unsupported` — see the disclosed tier-3 gap list in [pr-artifacts/README.md](pr-artifacts/README.md) |
