#!/bin/bash
# proc-build.sh -- link the std probe + streaming process glue into C:RustPath.
# into C:RustPath (the RSN std::net::TcpStream round-trip). Same recipe as
# net-build.sh; the only change is the harness (rs_path_main.c) and the output
...
echo "[path] compile glue + harness (-ffixed-x18)"
...
"$CC" "${CFLAGS[@]}" -c "$DIR/rs_path_main.c" -o "$OUT/rs_path_main.o"

echo "[path] link RustPath"
COMPILER_PATH="$XTBIN" "$COLLECT" \
    --eh-frame-hdr --allow-multiple-definition \
    -L"$LIBDIR" -L"$XTLIB" -o "$OUT/RustPath" \
    "$LIBDIR/startup.o" "$OUT/rs_path_main.o" "$OUT/aros_net_glue.o" "$OUT/aros_fs_glue.o" "$OUT/aros_process_glue.o" "$OUT/aros_proc_glue.o" "$OUT/aros_thread_glue.o" "$OUT/aros_sync_glue.o" "$OUT/aros_env_glue.o" "$RSLIB" \
    -\( "${AUTOLIB[@]}" "${STDLIBS[@]}" -\)
echo "[path] built: $OUT/RustPath ($(stat -f%z "$OUT/RustPath" 2>/dev/null) bytes)"
cp -f "$OUT/RustPath" "$CDIR/RustPath"; chmod +x "$CDIR/RustPath"
echo "[path] deployed -> $CDIR/RustPath"
