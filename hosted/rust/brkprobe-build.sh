#!/bin/bash
# brkprobe-build.sh -- build C:BrkProbe, the streaming-child-pipe reproducer.
#
...
# brkprobe_main.c for what it asserts.
set -eu
...
CC="${AROS_CC:-clang}"
OUT="$REPO/build/brkprobe"; mkdir -p "$OUT"
...
echo "[brkprobe] compile"
"$CC" "${CFLAGS[@]}" -c "$DIR/brkprobe_main.c" -o "$OUT/brkprobe_main.o"
"$CC" "${CFLAGS[@]}" -c "$DIR/aros_proc_glue.c" -o "$OUT/aros_proc_glue.o"

echo "[brkprobe] link"
COMPILER_PATH="$XTBIN" "$COLLECT" \
    --eh-frame-hdr --allow-multiple-definition \
    -L"$LIBDIR" -L"$XTLIB" -o "$OUT/BrkProbe" \
    "$LIBDIR/startup.o" "$OUT/brkprobe_main.o" "$OUT/aros_proc_glue.o" \
    -\( "${AUTOLIB[@]}" "${STDLIBS[@]}" -\)

cp -f "$OUT/BrkProbe" "$CDIR/BrkProbe"; chmod +x "$CDIR/BrkProbe"
echo "[brkprobe] deployed -> $CDIR/BrkProbe ($(stat -f%z "$OUT/BrkProbe") bytes)"
