#!/bin/bash
# stack-build.sh -- link the std probe into C:RustStack, the thread-stack check.
# Same recipe as alloc-build.sh; only the harness (rs_stack_main.c) and the
# output name differ.
set -eu
...
echo "[stack] compile glue + harness (-ffixed-x18)"
...
"$CC" "${CFLAGS[@]}" -c "$DIR/rs_stack_main.c" -o "$OUT/rs_stack_main.o"

echo "[stack] link RustStack"
COMPILER_PATH="$XTBIN" "$COLLECT" \
    --eh-frame-hdr --allow-multiple-definition \
    -L"$LIBDIR" -L"$XTLIB" -o "$OUT/RustStack" \
    "$LIBDIR/startup.o" "$OUT/rs_stack_main.o" "$OUT/aros_net_glue.o" "$OUT/aros_fs_glue.o" "$OUT/aros_process_glue.o" "$OUT/aros_proc_glue.o" "$OUT/aros_thread_glue.o" "$OUT/aros_sync_glue.o" "$OUT/aros_env_glue.o" "$RSLIB" \
    -\( "${AUTOLIB[@]}" "${STDLIBS[@]}" -\)
echo "[stack] built: $OUT/RustStack ($(stat -f%z "$OUT/RustStack" 2>/dev/null) bytes)"
cp -f "$OUT/RustStack" "$CDIR/RustStack"; chmod +x "$CDIR/RustStack"
echo "[stack] deployed -> $CDIR/RustStack"
