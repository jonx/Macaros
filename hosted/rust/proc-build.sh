#!/bin/bash
# proc-build.sh -- link the std probe + streaming process glue into C:RustProc.
# into C:RustProc (the RSN std::net::TcpStream round-trip). Same recipe as
# net-build.sh; the only change is the harness (rs_proc_main.c) and the output
...
echo "[proc] compile glue + harness (-ffixed-x18)"
...
"$CC" "${CFLAGS[@]}" -c "$DIR/rs_proc_main.c" -o "$OUT/rs_proc_main.o"

echo "[proc] link RustProc"
COMPILER_PATH="$XTBIN" "$COLLECT" \
    --eh-frame-hdr --allow-multiple-definition \
    -L"$LIBDIR" -L"$XTLIB" -o "$OUT/RustProc" \
    "$LIBDIR/startup.o" "$OUT/rs_proc_main.o" "$OUT/aros_net_glue.o" "$OUT/aros_fs_glue.o" "$OUT/aros_process_glue.o" "$OUT/aros_proc_glue.o" "$OUT/aros_thread_glue.o" "$OUT/aros_sync_glue.o" "$OUT/aros_env_glue.o" "$RSLIB" \
    -\( "${AUTOLIB[@]}" "${STDLIBS[@]}" -\)
echo "[proc] built: $OUT/RustProc ($(stat -f%z "$OUT/RustProc" 2>/dev/null) bytes)"
cp -f "$OUT/RustProc" "$CDIR/RustProc"; chmod +x "$CDIR/RustProc"
echo "[proc] deployed -> $CDIR/RustProc"
