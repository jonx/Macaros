#!/bin/bash
# net-build.sh -- link the std probe + the bsdsocket net glue + rs_net_main.c into
# C:RustNet (the RS4 TCP-round-trip test). Same recipe as std-build.sh, plus:
#   - the AmiTCP network headers (sys/socket.h, netinet/in.h) from the OS source
#   - -ffixed-x18 on the C glue so the bridge itself can't hit the x18 clobber
set -eu
...
# -ffixed-x18: the bridge code itself must never park a value in the reserved reg.
...
COMPILER_PATH="$XTBIN" "$COLLECT" \
    ...
    "$LIBDIR/startup.o" "$OUT/rs_net_main.o" "$OUT/aros_net_glue.o" "$OUT/aros_fs_glue.o" "$OUT/aros_process_glue.o" "$OUT/aros_thread_glue.o" "$OUT/aros_sync_glue.o" "$RSLIB" \
    -\( "${AUTOLIB[@]}" "${STDLIBS[@]}" -\)
echo "[net] built: $OUT/RustNet ($(stat -f%z "$OUT/RustNet" 2>/dev/null) bytes)"
cp -f "$OUT/RustNet" "$CDIR/RustNet"; chmod +x "$CDIR/RustNet"
echo "[net] deployed -> $CDIR/RustNet"
