#!/bin/bash
# stack-build.sh -- link the std probe into C:RustShell, the shell-over-pipes check.
# Same recipe as alloc-build.sh; only the harness (rs_shell_main.c) and the
# output name differ.
set -eu
...
echo "[shell] compile glue + harness (-ffixed-x18)"
...
"$CC" "${CFLAGS[@]}" -c "$DIR/rs_shell_main.c" -o "$OUT/rs_shell_main.o"

echo "[shell] link RustShell"
COMPILER_PATH="$XTBIN" "$COLLECT" \
    --eh-frame-hdr --allow-multiple-definition \
    -L"$LIBDIR" -L"$XTLIB" -o "$OUT/RustShell" \
    "$LIBDIR/startup.o" "$OUT/rs_shell_main.o" "$OUT/aros_net_glue.o" "$OUT/aros_fs_glue.o" "$OUT/aros_process_glue.o" "$OUT/aros_proc_glue.o" "$OUT/aros_thread_glue.o" "$OUT/aros_sync_glue.o" "$OUT/aros_env_glue.o" "$RSLIB" \
    -\( "${AUTOLIB[@]}" "${STDLIBS[@]}" -\)
echo "[shell] built: $OUT/RustShell ($(stat -f%z "$OUT/RustShell" 2>/dev/null) bytes)"
cp -f "$OUT/RustShell" "$CDIR/RustShell"; chmod +x "$CDIR/RustShell"
echo "[shell] deployed -> $CDIR/RustShell"
