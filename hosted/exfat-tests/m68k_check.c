/*
 * Gate 1, m68k half: compile the production headers for a 32-bit big-endian
 * target with no hosted C library.
 *
 * The host tests prove the arithmetic on aarch64. This proves the same
 * headers survive the target where the claims actually bite: 32-bit long,
 * big-endian byte order, and a 68000 that takes an address error rather than
 * a slow path on an odd-address word access.
 *
 * Compile only. There is nothing to run here; if it compiles clean under
 * -Werror -Wconversion for -m68000, the header's assumptions hold.
 */
    ...
    UQUAD start, UQUAD total, UQUAD num, ULONG nblocks)
{
     ...
     */
    acc += exfat_rd16(boot, 3);
    acc += exfat_rd32(boot, 3);
    acc += (ULONG)(exfat_rd64(boot, EXFAT_BOOT_VOLUMELENGTH) & 0xFFFFu);
    acc += (ULONG)(exfat_rd64(boot, 1) & 0xFFu);
