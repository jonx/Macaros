...
# bare-binary model). This packages the same pieces as a double-clickable AROS.app:
#   Contents/MacOS/AROS              launcher script (CFBundleExecutable) — sets env
...
#
#   ./make-aros-app.sh            build build/AROS.app from the discovered build
...
APP="${AROS_APP:-$ROOT/build/AROS.app}"

INFO_PLIST='<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
  <key>CFBundleName</key><string>AROS</string>
  <key>CFBundleDisplayName</key><string>AROS</string>
  <key>CFBundleIdentifier</key><string>org.aros.hosted</string>
  <key>CFBundleExecutable</key><string>AROS</string>
...
assemble() {   # assemble <bootd> <app>
    ...
    printf '%s' "$LAUNCHER"   > "$_app/Contents/MacOS/AROS"; chmod +x "$_app/Contents/MacOS/AROS"
...
if [ "${1:-}" = "--check" ]; then
    ...
    assemble "$BD" "$TMP/AROS.app"
    A="$TMP/AROS.app/Contents"
    plutil -lint "$A/Info.plist" >/dev/null 2>&1; ck $? "Info.plist is valid"
    [ -x "$A/MacOS/AROS" ];                       ck $? "MacOS/AROS launcher present + executable"
    ...
    grep -q 'AROS_DARWIN_THREADED=1' "$A/MacOS/AROS";       ck $? "launcher sets AROS_DARWIN_THREADED"
    grep -q 'AROS_SETTINGS_SCHEMA' "$A/MacOS/AROS";         ck $? "launcher points the dylib at the bundled schema"
    grep -q 'aros-host-conf.sh' "$A/MacOS/AROS";            ck $? "launcher sources aros-host-conf.sh"
...
echo ">> built $APP"
echo ">> open it:  open '$APP'      (or: '$APP/Contents/MacOS/AROS' to see stdout)"
